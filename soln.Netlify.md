---
id: oS5Bdyxb1v6EpXu2rIBM5
title: Netlify
desc: serverless deployment platform for web sites
updated: 1638033458861
created: 1638031977724
---



- [[p.vs]] [[soln.Vercel]]  ^uUZD1OtrpGRW
  - [[p.hasRelatedResource]] https://dev.to/maxniederman/netlify-vs-vercel-a-comparison-5643
    - Netlify bills based on the number of invocations, whereas Vercel bills based on GB-hours since you can customize your serveless function instances. 
- [[p.hasFeature]] 
  - [[t.cs.serverless]]
    - [[p.hasClientSupport]] [[t.cs.language.JavaScript]] [[t.cs.language.go]]
  - [[t.cs.authentication]] via built-in authentication platform based on the [[GoTrue|soln.GoTrue]] API 
  - Netlify Forms
  - Netlify Analytics (Paid)
  - Split Testing
  - Allows commercial use
